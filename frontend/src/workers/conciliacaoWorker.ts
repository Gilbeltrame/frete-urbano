...
export interface WorkerMessage {
	type: "PROCESS_DATA" | "PROGRESS_UPDATE" | "VALIDATION_COMPLETE" | "ERROR";
	payload?: any;
}
...
class ConciliacaoWorker {
	...
	private async calcularFreteMinimo(params: {
		tabela: string;
		tipoCarga: string;
		eixos: number;
		distanciaKm: number;
	...
	public async processarDados(dados: OrcamentoItem[]): Promise<void> {
		...
		try {
			...
			// Chamar API de cálculo em massa
				...
				if (response.ok) {
					...
					self.postMessage({
						...
						payload: {
							...
							processed: 0,
							total,
							currentItem: `Cálculos recebidos: ${calculosAPI.resumo?.sucessos || 0} sucessos, ${calculosAPI.resumo?.erros || 0} erros`,
						},
					});
				}
			} catch (error) {
				console.warn("Falha na API de cálculo em massa, processando individualmente:", error);
			}
			...
			for (let i = 0; i < total; i++) {
				...
				// Obter resultado do cálculo da API (se disponível)
				let calculoAPI = null;
				if (calculosAPI?.resultados?.[i]?.status === "sucesso") {
					calculoAPI = calculosAPI.resultados[i];
				}
	...
	private async analisarItemComCalculo(item: OrcamentoItem, calculoAPI: any): Promise<ConciliacaoResult> {
		...
		if (calculoAPI?.freteMinimo) {
			...
			return await this.analisarItem(item);
		}
